...
def features(stentence):
    ...
    for i in range(len(u_0_0)):
        w = u_0_0[i - 1] + '/' + u_0_0[i] if i else '_B-1/' + u_0_0[i]
        u_1_0.append(w)
...
def viterbi(stentence, uni_fea={}, bi_fea={}, weights={}, use_bi=False, states=['B', 'E', 'M', 'S']):
    ...
    f_u00, f_u10 = features(stentence)
    ...
    for i in range(len(f_u00)):
        ...
        for w in range(length):
            w_u00 = weights[uni_fea[f_u00[i]]+w] if f_u00[i] in uni_fea else 0.0
            w_u10 = weights[uni_fea[f_u10[i]]+w] if f_u10[i] in uni_fea else 0.0

            state_weights[w] = w_u00 + w_u10

        # 转移特征 f(y_i_1,y_i,x_i)
        word = f_u00[i]

        bi_id = bi_fea[word] if use_bi else 0
